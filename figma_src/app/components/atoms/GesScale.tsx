 ...
 *
 * Remplace EchelleGes (raw Figma import)
...
export function GesScale({ activeClass, theme = "light", size = "md" }: GesScaleProps) {
  const h = size === "sm" ? "h-3" : "h-5";
  return (
    <div className="flex items-end gap-[2px]" aria-label={`GES: ${activeClass ?? 'non renseigné'}`}>
      {GES_CLASSES.map((cls) => (
