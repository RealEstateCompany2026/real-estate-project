...
import { Chip, ChipDate, ChipId } from "../atoms/Chip";
...
function AppBarContainer({ children, className = "" }: AppBarContainerProps) {
  return (
    <div
      className={`flex items-center px-[20px] ${className}`.trim()}
      style={{
        height: "100px",
...
function AppBarTitle({ children }: AppBarTitleProps) {
  return (
    <h4
      className="font-bold text-[28px] leading-[34px] tracking-[0.28px] whitespace-nowrap px-[10px]"
      style={{
  ...
  return (
    <AppBarContainer className={className}>
      <div className="flex items-center gap-[8px] flex-1">
        ...
        {fileName && (
          <span
            className="font-semibold text-[16px] leading-[20px] tracking-[0.16px] px-[10px] py-[8px]"
            style={{
              fontFamily: "Roboto, sans-serif",
              color: "var(--text-body)",
            }}
      ...
      <button
        onClick={onSave}
        disabled={saving}
        className="flex items-center gap-[8px] px-[12px] py-[12px] rounded-[16px] transition-all hover:opacity-90 disabled:opacity-50"
        style={{
    ...
    <AppBarContainer className={className}>
      <div className="flex items-center flex-1" style={{ gap: gapSize }}>
        ...
        {badges.map((badge, index) => (
          <Badge key={index} label={badge.label} variant={badge.variant || "default"} />
        ))}
