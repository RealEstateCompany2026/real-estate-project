...
/**
 * ListCandidatureLocataire - Ligne de liste candidature locataire
 ...
 *   - Droite : badge dossier + badge décision + "Voir le dossier >" + AI
 *
 * Figma : "List . CandidatureLocataire" — h=70px, px=20, py=13, justify-between
 */

export interface CandidatureLocataireStatus {
  /** Badge dossier (ex: COMPLET, INCOMPLET) */
  dossier: { label: string; variant: BadgeVariant };
  /** Badge décision (ex: ACCEPTÉ, REFUSÉ, EN ATTENTE) */
  decision: { label: string; variant: BadgeVariant };
}
...
export interface ListCandidatureLocataireProps {
  ...
  status: CandidatureLocataireStatus;
  /** Nombre de suggestions IA */
  aiSuggestions?: number;
  /** Callback au clic sur "Voir le dossier" */
  onView?: () => void;
...
export function ListCandidatureLocataire({
  ...
  return (
    ...
    >
      ...
      {/* Droite : badges statut + bouton + AI */}
      <div className="flex gap-[24px] items-center shrink-0">
        <Badge variant={status.dossier.variant}>{status.dossier.label}</Badge>
        <Badge variant={status.decision.variant}>{status.decision.label}</Badge>
